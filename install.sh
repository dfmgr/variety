...
# @@File             :  install.sh
# @@Description      :  Install configurations for variety
...
__run_pre_install() {
  is_running="$(ps aux | grep "variety" | grep -v ' grep' | grep -q '^' && variety -q &>/dev/null && sleep 10 echo 'true' || echo '')"
  was_running="$is_running"
  [ -n "$is_running" ] && killall variety && is_running="false" && was_running="true" || true
  return ${?:-0}
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__run_post_install() {
  if [ "$DESKTOP_SESSION" = "xfce4" ] && [ "$is_running" = "false" ] || [ "$was_running" = "true" ]; then
    variety &
    disown
  fi
  return ${?:-0}
